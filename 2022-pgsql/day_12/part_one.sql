create table grid (x int, y int, v int, is_start bool, is_end bool, primary key (x, y));

insert into grid
select (i - 1) % 95, (i - 1) / 95, ascii(translate(v, 'SE', 'az')), v = 'S', v = 'E'
from unnest(regexp_split_to_array(replace(pg_read_file('/data/day_12/input.txt'), E'\n', ''), '')) with ordinality _(v, i);

create table step (i int, x int, y int);

insert into step
select 0, x, y from grid where is_start;
