...
with
    packet(d) as (
        select unnest(regexp_split_to_array(replace(replace(trim(pg_read_file('/data/day_13/input.txt'), E'\n'), '10', 'a'), ',', ''), '\n+'))
        union all
        values ('[[2]]'), ('[[6]]')
    ),
    sorted as (
        select row_number() over (order by sum(cmp(l.d, r.d))) as i, l.d
        from packet l, packet r
        group by l.d
    )
select round(exp(sum(ln(i)))) from sorted where d in ('[[2]]', '[[6]]');
